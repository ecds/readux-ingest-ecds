...
class LocalTest(TestCase):
    ...
    def mock_local(self, bundle, with_manifest=False, metadata={}, from_bulk=False):
        ...
        local = Local(image_server=self.image_server, metadata=metadata)
        local.save()
    ...
    def test_unzip_bundle(self):
        ...
        assert os.path.isfile(
            os.path.join(
                settings.INGEST_PROCESSING_DIR, f"{local.manifest.pid}_00000005.jpg"
            )
        )
        assert os.path.isfile(
            os.path.join(
                ...
                local.manifest.pid,
                f"{local.manifest.pid}_00000007.tsv",
            )
        )
    ...
    def test_ignoring_junk(self):
        ...
        with ZipFile(
            os.path.join(self.fixture_path, "bundle_with_junk.zip"), "r"
        ) as zip_ref:
            files_in_bundle = zip_ref.namelist()

        assert os.path.isfile(
            os.path.join(
                settings.INGEST_PROCESSING_DIR, f"{local.manifest.pid}_00000009.jpg"
            )
        )
        assert os.path.isfile(
            os.path.join(
                settings.INGEST_OCR_DIR,
                local.manifest.pid,
                f"{local.manifest.pid}_00000003.tsv",
            )
        )
        ...
        assert (
            os.path.isfile(
                os.path.join(
                    settings.INGEST_PROCESSING_DIR,
                    f"{local.manifest.pid}.000000010.jpg",
                )
            )
            is False
        )
    ...
    def test_creating_canvases(self):
        ...
        ocr_path = os.path.abspath(
            os.path.join(settings.INGEST_OCR_DIR, pid, f"{pid}_00000008.tsv")
        )
    ...
    def test_it_creates_manifest_with_metadata_property(self):
        metadata = {"pid": "808", "publisher": "Goodie Mob"}
    ...
    def test_create_related_links(self):
        ...
        assert any(
            link["@id"] == "https://github.com/ecds/readux/tree/develop"
        ...
        assert any(
            link["@id"]
            == "https://archive.org/download/cherokeehymnbook00boud/cherokeehymnbook00boud.pdf"
            for link in related_links
        )
